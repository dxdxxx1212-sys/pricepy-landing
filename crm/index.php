...
<tr onclick="location='view.php?id=<?=$r['id']?>'" style="cursor:pointer">
  ...
  <td><b><?=h($r['name']?:'—')?></b><?php if(isset($dups[$r['phone_norm']]) && $r['id']!=$dups[$r['phone_norm']]['mn']){ ?> <span class="badge" style="background:#ff8a5b" title="Этот номер уже обращался — есть более ранняя заявка">повтор</span><?php } ?><br><span class="muted"><?=h($r['contact'])?></span><?php if($r['channel']){ $chC=['whatsapp'=>'#22c55e','telegram'=>'#0ea5e9','max'=>'#8b5cf6','phone'=>'#9aa2ab'][$r['channel']]??'#9aa2ab'; ?> <span class="pill" style="color:<?=$chC?>;border:1px solid <?=$chC?>;background:transparent" title="Способ связи, который клиент выбрал в квизе">хочет: <?=h(crm_channel_label($r['channel']))?></span><?php } ?></td>
