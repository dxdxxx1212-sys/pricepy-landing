...
foreach ($blocks as $b) {
  ...
  if (!crm_contact_plausible($contact)) { $junk++; continue; }
  ...
  $data = [
    ...
    'channel'  => (crm_channel_norm($chl) ?: mb_strtolower($chl)), 'contact' => $contact, // единый ключ канала; незнакомое значение — как есть
  ];
